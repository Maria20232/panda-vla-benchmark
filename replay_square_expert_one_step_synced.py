...
def main():
    ...
    try:
        ...
        # Synchronize controller internals with the restored simulator state.
        robot = env.robots[0]

        robot.composite_controller.update_state()

        right_controller = robot.part_controllers["right"]
        right_controller.update_reference_data()
        right_controller.reset_goal(
            goal_update_mode="achieved"
        )

        gripper_controller = robot.part_controllers[
            "right_gripper"
        ]
        gripper_controller.reset_goal()
